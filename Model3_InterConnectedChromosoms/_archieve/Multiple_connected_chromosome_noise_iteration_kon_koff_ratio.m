...
noise = 0.05;
noise_b = 0.05;
Kct = 11.2;  % pN/µm centromee spring constant 
...
% l0 values to scan
r_values = [0.1 0.5 1 2 5 10];   % ratio = kon/koff
Ksum = 100;                           % keep kon+koff constant (same timescale)
l0 = 1; % natural length of the spring 
k = 10;  % interchromosome coupling value 
...
for g = 1:size(tau_groups, 1)
    ...
    for li = 1:length(r_values)
        r_val = r_values(li);
        r_key = matlab.lang.makeValidName(sprintf('r_%g', r_val));;
        ...
        errorbar(grouped.neighbor, grouped.mean_C_value, grouped.std_C_value, ...
            ...
            'DisplayName', sprintf('r = %.3g', r_val), ...
            'LineWidth', 2);
    end
...
for n = 1:Nchromosomes-1
    ...
    errorbar((grouped.tau/tau_step)*5, grouped.mean_C_value,grouped.std_C_value, ...
        '-o', 'Color', cmap(li, :),'MarkerSize',7,...
        'DisplayName', sprintf('r = %.3g', r_val), 'LineWidth', 2);
    end
    xlabel('Time lag (s)');
    ylabel('Avg. Norm Cross-Corr');
