...
tau_step   = round(5 / (dt * 60));     % steps per 5 s
tau_values = tau_step * (1:30);        % 5 s ... 150 s
tau_groups = reshape(tau_values, 10, []).';   % 3x10, row 1 is 5–50 s
...
Cij_max_abs = 1e4;    % "very very large" -> outside correlation bounds (allow tiny overshoot)
...
for a = Alpha_scales
    for Kct = Kct_values
        for iter = 1:N_iter
            ...
            % Coupling state
            flag_c_sum_accumulated = zeros(Nsteps, 1); %#ok<NASGU>
            ...
            if any(isnan(cL), 'all') || any(isnan(cR), 'all')
                ...
                % Pearson's Analysis and the peak ----
                ...
                results_all = [results_all; table( ...
                    ...
                    Kct, ...
                    double(iter), ...
                    mean(pearson_table.pearson_r,'omitnan'), ...
                    'VariableNames', {'alpha_scale','alpha_effective','Kct','iteration','peak'})];
            end
