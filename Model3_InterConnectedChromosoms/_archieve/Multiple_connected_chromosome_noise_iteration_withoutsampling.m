...
while valid_iterations < target_iterations
    ...
    sampling_interval = round((5/60) / dt);                 % steps per 5 s
    total_steps       = min(round(duration / dt), Nsteps);  % cap at Nsteps
    cL_sampled = cL(1:sampling_interval:total_steps, 1, :);
    cR_sampled = cR(1:sampling_interval:total_steps, 1, :);
    CMx_sampled = ((cL_sampled + cR_sampled) / 2);   % [Nsteps x Nchromosomes]
    CMx = reshape(CMx_sampled, [], Nchromosomes);  
    Nsteps = size(CMx, 1); 
