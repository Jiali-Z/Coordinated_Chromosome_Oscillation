%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
% PURPOSE (Figure S4C)
%   This script generates time-series plots of chromosome connectivity for
%   specified (kon_0, koff_0) values, showing how the time-averaged number
%   of active inter-chromosomal connections evolves over time during a
%   single stochastic simulation.
%
%   For each chosen (kon_0, koff_0) pair, the total number of connected
%   chromosome pairs is computed at each time step, averaged over a sliding
%   time window, and plotted as a function of time.
%
% USAGE
%   Provide one or more kon_0 and koff_0 values as lists (kon0_list, koff0_list).
%   The script will simulate and plot the time evolution of window-averaged
%   connectivity for every (kon_0, koff_0) combination on the same figure
%   (one curve per parameter combination) 
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
...
% --------------------- USER SETTINGS ---------------------
koff0_list  = [40];         % list of koff_0 values to simulate
kon0_list   = [20];         % list of kon_0 values to simulate

avg_win_sec = 5;            % sliding window size (seconds) for averaging connectivity
ylim_max    = 10;           % y-axis max (5 chromosomes -> max 10 connections)
...
% Inter-chromosomal spring parameters (fixed in this script)
l0 = 3;    % µm
...
ylim([0, ylim_max]);
% Colormap for plotting multiple traces
...
for iKon = 1:numel(kon0_list)
    ...
    for iKoff = 1:numel(koff0_list)
        ...
        % Plot one curve per (kon_0, koff_0) combination
        line_width = 2;             % line width for time-series traces
        marker_every = 400;         % plot marker every N points (0 to disable)
        lbl = sprintf('kon_0=%g, koff_0=%g', kon_0, koff_0);
